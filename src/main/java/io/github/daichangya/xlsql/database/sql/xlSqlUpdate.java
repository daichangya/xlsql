...
/**
 * Represents an UPDATE SQL command.
 * Handles updates to rows in Excel tables.
 * 
 * @author daichangya
 */
public class xlSqlUpdate implements ICommand {
     ...
     */
    public xlSqlUpdate(io.github.daichangya.xlsql.database.ADatabase database, String schema, String table) {
        // 验证并设置数据库对象
        if (database == null) {
     ...
     * 检查UPDATE操作是否允许执行
     * 
     * <p>UPDATE操作始终允许执行，因为更新操作不会导致表超出限制。</p>
     *
     * @return 始终返回true
