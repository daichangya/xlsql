 ...
 * H2与HSQLDB在SQL语法上高度兼容，支持相同的表名引用格式（双引号）。
...
public class xlH2Select extends ASqlSelect {
     ...
     * - 否则查询格式为：SELECT * FROM "workbook.sheet"
     * 
     * H2与HSQLDB使用相同的表名引用格式（双引号），因此SQL语句格式完全兼容。</p>
     * 
    ...
    public ResultSet QueryData(String workbook, String sheet) 
        ...
        String sql;
        // H2使用双引号包围表名，支持"workbook.sheet"格式（与HSQLDB相同）
