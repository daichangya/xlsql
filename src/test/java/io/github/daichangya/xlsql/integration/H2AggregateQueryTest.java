...
import io.github.daichangya.xlsql.base.H2EngineTestBase;

/**
 * H2AggregateQueryTest - H2聚合查询测试
 * 
 * <p>测试H2引擎的聚合功能，包括：
 * <ul>
 *   <li>COUNT聚合</li>
...
@Tag("h2")
@Tag("aggregate")
public class H2AggregateQueryTest extends H2EngineTestBase {
    ...
    public void testSumFunction() throws Exception {
        try (Statement stmt = con.createStatement();
             ResultSet rs = stmt.executeQuery(
                 "SELECT SUM(age) AS sum_a FROM test1_Sheet1 WHERE a IS NOT NULL")) {
            ...
            if (e.getMessage() != null && 
                (e.getMessage().contains("not found") || 
                 e.getMessage().contains("Data conversion"))) {
    ...
    public void testAvgFunction() throws Exception {
        try (Statement stmt = con.createStatement();
             ResultSet rs = stmt.executeQuery(
                 "SELECT AVG(age) AS avg_a FROM test1_Sheet1 WHERE a IS NOT NULL")) {
    ...
    public void testGroupByMultipleColumns() throws Exception {
        try (Statement stmt = con.createStatement();
            ...
            if (e.getMessage() != null && 
                (e.getMessage().contains("not found") || 
                 e.getMessage().contains("Column \"B\" not found"))) {
