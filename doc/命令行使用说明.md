...
```bash
java -jar target/xlsql-5.1.1.jar
```

或者使用包含所有依赖的 Shaded JAR：

```bash
java -jar target/xlsql-5.1.1-shaded.jar
```
...
-export test1_Sheet1 AS hsqldb TO jdbc_connection
...
# 插入数据
INSERT INTO test1_Sheet1 (column1, column2) VALUES ('value1', 'value2');

# 更新数据
UPDATE test1_Sheet1 SET column1 = 'new_value' WHERE column2 = 'condition';

# 删除数据
DELETE FROM test1_Sheet1 WHERE column1 = 'value';
...
# 导出特定表为HSQLDB格式到数据库连接
-export test_Sheet1 AS hsqldb TO out
