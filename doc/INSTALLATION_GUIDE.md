...
生成的文件：
- `target/xlsql-5.0-SNAPSHOT.jar` - 标准 JAR（不包含依赖）
- `target/xlsql-5.0-SNAPSHOT-shaded.jar` - Fat JAR（包含所有依赖）
...
生成文件：`target/xlsql-5.0-SNAPSHOT-sources.jar`
...
生成文件：`target/xlsql-5.0-SNAPSHOT-javadoc.jar`
...
**文件**: `target/xlsql-5.0-SNAPSHOT.jar`
...
**文件**: `target/xlsql-5.0-SNAPSHOT-shaded.jar`
...
**文件**: `target/xlsql-5.0-SNAPSHOT-sources.jar`
...
**文件**: `target/xlsql-5.0-SNAPSHOT-javadoc.jar`
...
2. 运行测试
3. 打包 JAR
4. 安装到本地 Maven 仓库：`~/.m2/repository/com/jsdiff/xlsql/5.0-SNAPSHOT/`
...
1. 复制 JAR 到项目：
```bash
cp target/xlsql-5.0-SNAPSHOT.jar /path/to/your/project/lib/
```
...
3. 编译时包含在 classpath：
```bash
javac -cp "lib/xlsql-5.0-SNAPSHOT.jar:lib/poi-5.2.3.jar:..." YourClass.java
```
...
1. 复制 Shaded JAR：
```bash
cp target/xlsql-5.0-SNAPSHOT-shaded.jar /path/to/your/project/lib/
```

2. 编译和运行：
```bash
# 编译
javac -cp "lib/xlsql-5.0-SNAPSHOT-shaded.jar" YourClass.java

# 运行
java -cp "lib/xlsql-5.0-SNAPSHOT-shaded.jar:." YourClass
```
...
```bash
java -jar xlsql-5.0-SNAPSHOT-shaded.jar [options]
```
...
```bash
# 检查本地仓库
ls ~/.m2/repository/com/jsdiff/xlsql/5.0-SNAPSHOT/

# 应该看到：
# - xlsql-5.0-SNAPSHOT.jar
# - xlsql-5.0-SNAPSHOT.pom
# - xlsql-5.0-SNAPSHOT-sources.jar (如果生成了)
# - xlsql-5.0-SNAPSHOT-javadoc.jar (如果生成了)
```
...
```bash
# 查看 JAR 内容
jar -tf target/xlsql-5.0-SNAPSHOT.jar | head -20

# 查看 Shaded JAR 大小
ls -lh target/xlsql-5.0-SNAPSHOT-shaded.jar
```
...
```bash
javac -cp "target/xlsql-5.0-SNAPSHOT-shaded.jar" TestConnection.java
java -cp "target/xlsql-5.0-SNAPSHOT-shaded.jar:." TestConnection
```
