...
生成的文件：
- `target/xlsql-5.1.1.jar` - 标准 JAR（不包含依赖）
- `target/xlsql-5.1.1-shaded.jar` - Fat JAR（包含所有依赖）
...
生成文件：`target/xlsql-5.1.1-sources.jar`
...
生成文件：`target/xlsql-5.1.1-javadoc.jar`
...
**文件**: `target/xlsql-5.1.1-shaded.jar`
...
**文件**: `target/xlsql-5.1.1-sources.jar`
...
**文件**: `target/xlsql-5.1.1-javadoc.jar`
...
1. 复制 JAR 到项目：
```bash
cp target/xlsql-5.1.1.jar /path/to/your/project/lib/
```
...
3. 编译时包含在 classpath：
```bash
javac -cp "lib/xlsql-5.1.1.jar:lib/poi-5.2.3.jar:..." YourClass.java
```
...
1. 复制 Shaded JAR：
```bash
cp target/xlsql-5.1.1-shaded.jar /path/to/your/project/lib/
```

2. 编译和运行：
```bash
# 编译
javac -cp "lib/xlsql-5.1.1-shaded.jar" YourClass.java

# 运行
java -cp "lib/xlsql-5.1.1-shaded.jar:." YourClass
```
...
```bash
java -jar xlsql-5.1.1-shaded.jar [options]
```
...
# - xlsql-5.1.1.jar
# - xlsql-5.1.1.pom
# - xlsql-5.1.1-sources.jar (如果生成了)
# - xlsql-5.1.1-javadoc.jar (如果生成了)
```
...
```bash
# 查看 JAR 内容
jar -tf target/xlsql-5.1.1.jar | head -20

# 查看 Shaded JAR 大小
ls -lh target/xlsql-5.1.1-shaded.jar
```
...
```bash
javac -cp "target/xlsql-5.1.1-shaded.jar" TestConnection.java
java -cp "target/xlsql-5.1.1-shaded.jar:." TestConnection
```
